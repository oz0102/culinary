//\src\app\continental\[id]\page.tsx
'use client'
...
import { continentalDishes } from '@/lib/data-utils'
import { 
  LandscapeTechnique, 
  FreeFormTechnique, 
...
export default function DishDetail({ params }: { params: { id: string } }) {
  ...
  // Find the dish by ID
  const dish = continentalDishes.find(d => d.id === params.id)
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <div className="container py-12 flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <Button onClick={() => router.push('/continental')}>
              Back to Continental Dishes
            </Button>
          </div>
        </div>
      </div>
  ...
  const relatedDishes = dish.relatedDishes
    .map(id => continentalDishes.find(d => d.id === id))
    .filter(Boolean)
                ...
                <div className="space-y-4">
                  {relatedDishes.map(related => related && (
                    <div 
                      ...
                      onClick={() => router.push(`/continental/${related.id}`)}
                    >
